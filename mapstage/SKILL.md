...
`git clone` of this GitHub repo pulls the **whole project** (README, showcases, tuner, skill). The agent skill is only the `mapstage/` folder — copy it into the host skills directory (e.g. `~/.grok/skills/mapstage`) and reload. Agents do not auto-discover `mapstage/SKILL.md` from a clone.
